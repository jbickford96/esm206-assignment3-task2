...
**Note: You should get a warning about one piece dropped. What happens to the name in Row 49, which contains two dashes? The name after the second dash disappears!

```{r}
sw_split_name <- starwars %>% #Start with starwars data frame
...
```{r}
sw_skin_sep <- starwars %>% #Start from starwards dataframe
  separate(col = skin_color, #Separate 'skin color' column
           into = c("skin_1", "skin_2"), #Into two columns, skin_1 and skin_2
           sep = ",", #Separate at the first comma 
           extra = "merge", #Merge additional skin colors, if any into second column
...
some_dates <- c(12301956, 11041950, 08311996) # Creates a vector of dates w/out spaces
some_dates_converted <- mdy(some_dates) # Converts to Date class in YYYY-MM-DD format, stores as some_dates_converted
...
points_by_month <- lakers_dates %>% # Start from lakers_dates
  group_by(game_type, month_new) %>% # Group by game_type and month_new
  summarize(total_points = sum(points)) # Sums the points by game type and month in new column total_points
...
storms_danger <- storms %>% 
  mutate(danger = case_when(  #new column 'danger' added
    status == "hurricane" ~ "high", #labels all observations of 'hurricane' in new 'danger' column
    TRUE ~ "moderate" #all observations not 'hurricane' labeled 'moderate'
    ))
```
